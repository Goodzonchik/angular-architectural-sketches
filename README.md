...
1. Настроить lint staged + prettier на прекоммит (В РБС такого нет, в форе был только преттиер).)
   Почему:
...
2. Делать state-сервисы для сущностей, а компоненты максимально использовать для отрисовки
...
4. Писать свою сетку стилей вместо Bootstrap и аналогов
   Почему:
...
6. Обязательно выносить SVG-иконки в отдельный компонент
   Почему:
...
7. Перейти на названия по Angular
...
9. Подключить GIT ACtions для понта и с иконкой статуса
