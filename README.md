...
**Почему:** при генерации нового компонента название формируется следующим образом "app-component-name". Так как он не несет смысловой нагрузки, то удаляется в ручную у каждого компонента.
...
- Для форматирования кода не потребуется, чтобы у всех разработчиков были одинаковые настройки форматирования в IDE, хук сделает работу за разработчика
...
- Импортировать функцию проще и правильнее, чем использовать сервис в некоторых ситуациях
